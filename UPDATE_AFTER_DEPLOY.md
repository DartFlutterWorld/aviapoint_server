# 🚀 Обновление сервера после заливки нового кода

## Шаг 1: Пересобрать и перезапустить Docker контейнер

После заливки нового кода **обязательно** нужно пересобрать образ и перезапустить контейнер, так как код компилируется в Docker образ.

### Для продакшена (docker-compose.prod.yaml):

```bash
# На сервере
cd /home/aviapoint_server  # или путь к вашему проекту

# 1. Пересобрать образ приложения
docker-compose -f docker-compose.prod.yaml build app

# 2. Перезапустить контейнер приложения
docker-compose -f docker-compose.prod.yaml up -d --no-deps app

# 3. Проверить логи
docker-compose -f docker-compose.prod.yaml logs -f app
```

### Для локальной разработки (docker-compose.dev.yaml):

```bash
# В директории проекта
# Запускаем только БД и Adminer (сервер запускается отдельно через dart run)
docker-compose -f docker-compose.dev.yaml up -d
docker-compose -f docker-compose.dev.yaml logs -f
```

---

## Шаг 2: Запустить миграции БД

После обновления кода нужно применить миграции БД, если были изменения в структуре таблиц.

### Новая миграция (удаление полей подписки из profiles):

```bash
# Подключитесь к БД и выполните миграцию
psql -h localhost -U postgres -d aviapoint -f migrations/remove_subscription_fields_from_profiles.sql
```

Или через Adminer (http://localhost:8082):
1. Откройте Adminer
2. Выберите базу `aviapoint`
3. Перейдите в "SQL-запрос"
4. Скопируйте содержимое `migrations/remove_subscription_fields_from_profiles.sql`
5. Выполните запрос

### Проверка других миграций:

Если есть другие новые миграции, выполните их в правильном порядке:

```bash
# Проверьте, какие миграции еще не применены
# Обычно миграции идут в таком порядке:
psql -h localhost -U postgres -d aviapoint -f migrations/create_payments_table.sql
psql -h localhost -U postgres -d aviapoint -f migrations/create_subscriptions_table.sql
psql -h localhost -U postgres -d aviapoint -f migrations/add_subscription_fields_to_payments.sql
psql -h localhost -U postgres -d aviapoint -f migrations/remove_subscription_fields_from_profiles.sql
```

---

## Шаг 3: Проверка работоспособности

### 1. Проверьте статус контейнеров:

```bash
docker-compose -f docker-compose.prod.yaml ps
```

Все контейнеры должны быть в статусе `Up` или `Up (healthy)`

### 2. Проверьте логи приложения:

```bash
docker-compose -f docker-compose.prod.yaml logs --tail=50 app
```

Ищите:
- ✅ `Server started on port 8080` - сервер запущен
- ❌ Ошибки подключения к БД
- ❌ Ошибки компиляции

### 3. Проверьте API:

```bash
# Проверьте, что API отвечает
curl http://localhost:8080/openapi/

# Или через nginx (если настроен)
curl http://localhost/openapi/
```

### 4. Проверьте структуру БД:

Убедитесь, что поля подписки удалены из таблицы `profiles`:

```sql
-- В Adminer или psql
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'profiles';
```

Не должно быть:
- `subscription_end_date`
- `has_active_subscription`
- `subscription_updated_at`

---

## Быстрая команда для обновления (все в одном):

```bash
# На сервере
cd /home/aviapoint_server

# 1. Пересобрать и перезапустить
docker-compose -f docker-compose.prod.yaml build app && \
docker-compose -f docker-compose.prod.yaml up -d --no-deps app

# 2. Запустить миграцию
psql -h localhost -U postgres -d aviapoint -f migrations/remove_subscription_fields_from_profiles.sql

# 3. Проверить логи
docker-compose -f docker-compose.prod.yaml logs --tail=20 app
```

---

## ⚠️ Важно!

1. **Всегда пересобирайте Docker образ** после изменения кода Dart
   - Код компилируется в бинарный файл внутри образа
   - Просто перезапуск контейнера не обновит код

2. **БД не нужно перезапускать** - она работает независимо

3. **Nginx не нужно перезапускать** - достаточно перезагрузки:
   ```bash
   docker exec aviapoint-nginx nginx -s reload
   ```

4. **Проверяйте логи** после каждого обновления

---

## Что НЕ нужно делать:

❌ Не нужно делать `docker-compose -f docker-compose.dev.yaml down` и `docker-compose -f docker-compose.dev.yaml up` - это остановит все контейнеры  
❌ Не нужно перезапускать БД  
❌ Не нужно перезапускать nginx (только перезагрузка)  
❌ Не нужно пересобирать все сервисы - только `app`

---

## Если что-то пошло не так:

### Откат к предыдущей версии:

```bash
# 1. Остановите текущий контейнер
docker-compose -f docker-compose.prod.yaml stop app

# 2. Вернитесь к предыдущему коммиту (если используете git)
git checkout HEAD~1

# 3. Пересоберите и запустите
docker-compose -f docker-compose.prod.yaml build app
docker-compose -f docker-compose.prod.yaml up -d app
```

### Просмотр старых логов:

```bash
docker-compose -f docker-compose.prod.yaml logs --tail=100 app
```

---

**Готово!** После выполнения этих шагов ваше приложение будет обновлено! 🎉

